...
int main(int argc, char *argv[])
{
    energy_par *en_par;
    mc_traj_data *mc_trj;
    histogram *psi_phi = histogram_init  (-M_PI, M_PI, 180, 0.5    );
    histogram_add_dimension(-M_PI, M_PI, 180, psi_phi);

    double *point;
    point = (double*)calloc(2, sizeof(double));

		mc_move_data *mc_mvdt;
		Init_MC(&mc_mvdt, &mc_trj, &en_par, "unused",argv[1]);
		//MAIN LOOP
		FILE *dihed_out=fopen("dihedrals.dat","w");
		for(int i=0;i<5000000;i++)
		{
			if(i>0 && i%50==0) {
				for (int j=1;j<mc_trj->new.p->n_res-1;j++) {
					fprintf(dihed_out,"%lf %lf  ",mc_trj->new.p->phi[j],mc_trj->new.p->psi[j]);
				}
				fprintf(dihed_out,"\n");
				/*
				fprintf(stdout,"%d --\n",i);
			...
			if(i%2000==0){
                for (size_t resi=3; resi < mc_trj->new.p->n_res-3; resi++)
                {
			...
			if(i<5000 || gsl_rng_uniform(mc_trj->rng_r)>0.7) //first 1000 steps equil
			{
				CATMV_pivot(mc_mvdt,mc_trj->new.p,mc_trj->rng_r,mc_trj->new.p->n_res/2);
			} else if (gsl_rng_uniform(mc_trj->rng_r)>0.5) {
				//CATMV_cranck(mc_mvdt,mc_trj->new.p,mc_trj->rng_r);
				CATMV_concerted_rot(mc_mvdt,mc_trj->new.p,mc_trj->rng_r,0.4);
			} else {
				//CATMV_cranck(mc_mvdt,mc_trj->new.p,mc_trj->rng_r);
				CATMV_concerted_rot(mc_mvdt,mc_trj->new.p,mc_trj->rng_r,0.1);
			}
			Compute_energy_new(mc_mvdt,mc_trj,en_par);
			Metropolis(mc_trj);
			if(i%10000==0)
			{
				//PEZZOTTO PER VERIFICARE CHE I CB SIANO CORRETTI
				//for(int i=0;i<mc_trj->new.p->n_res;i++) {
				//	CAT_insert_cbeta(mc_trj->new.p,i,CAT_Cb_AZIMUTH,CAT_Rbond_CCb);
				//}
				//CATIO_cat2pdb("prova_check_CB.pdb","a","--",mc_trj->new.p,1);
				//for(int i=0;i<mc_trj->new.p->n_res;i++) {
				//	mc_trj->new.p->CB[i][0]=mc_trj->old.p->CB[i][0];
				//	mc_trj->new.p->CB[i][1]=mc_trj->old.p->CB[i][1];
				//	mc_trj->new.p->CB[i][2]=mc_trj->old.p->CB[i][2];
				//}
				//FINE PEZZOTTO
		...
		free(en_par->M);
		free(en_par);
        histogram_print("test.dat", "matrix2d", psi_phi);
	...
	for(int i=0;i<p->n_res;i++) {
		for(int j=0;j<p->n_res;j++) {
			...
			if(d>2) {
				...
				// Yep, thanks Mirek!
				//if(p->CB!=0) { 
					//e_SAW += CATENR_Saw_g(p->CB[i], p->CB[j], CATENR_SAW_CB_CB);
					//e_SAW += CATENR_Saw_g(p->CB[i], p->CA[j], CATENR_SAW_CB_CA);
				//}
				if(e_SAW>0) {
				...
				e_SAW += CATENR_Saw_g(p->C[i], p->C[j], CATENR_SAW_C_C); // C_{i} - C_{i+1, i-1}
				if (p->CB!=NULL)
				{
					e_SAW += CATENR_Saw_g(p->CB[i], p->CB[j], CATENR_SAW_CB_CB); // CB_{i} - CB_{i+1, i-1}
					e_SAW += CATENR_Saw_g(p->CB[i], p->N[j], CATENR_SAW_CB_N); // CB_{i} - N_{i+1, i-1}
					e_SAW += CATENR_Saw_g(p->CB[j], p->C[i], CATENR_SAW_CB_C); // CB_{i} - C_{i+1, i-1}
				}
			} else if (i==j && p->CB!= NULL) {
				e_SAW += CATENR_Saw_g(p->O[i], p->CB[j], CATENR_SAW_CB_O); // O_{i} - CB_{i}
			}
			if(e_SAW>0) {
...
void Compute_delta_en(mc_move_data *mvdt,conf *C, energy_par *ep)
{
	...
	for(int i=0;i<N;i++) {
		...
		if(d>2) {
			...
			//   			e_SAW += CATENR_Saw_g(p->CB[r1], p->CB[r2], CATENR_SAW_CB_CB); 
			if(e_SAW>0) {
			...
			e_SAW += CATENR_Saw_g(p->C[r1], p->C[r2], CATENR_SAW_C_C); // C_{i} - C_{i+1, i-1}
			if (p->CB!=NULL) {
				e_SAW += CATENR_Saw_g(p->CB[r1], p->CB[r2], CATENR_SAW_CB_CB); // CB_{i} - CB_{i+1, i-1}
				e_SAW += CATENR_Saw_g(p->CB[r1], p->N[r2], CATENR_SAW_CB_N); // CB_{i} - N_{i+1, i-1}
				e_SAW += CATENR_Saw_g(p->CB[r2], p->C[r1], CATENR_SAW_CB_C); // CB_{i} - C_{i+1, i-1}
			}
		} else if( p->CB!=NULL && r1==r2) {
			e_SAW += CATENR_Saw_g(p->O[r1], p->CB[r2], CATENR_SAW_CB_O); // O_{i} - CB_{i}
		}
		if(e_SAW>0) {
			break;
		}
